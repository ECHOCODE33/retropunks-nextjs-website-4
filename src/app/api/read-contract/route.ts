...
export async function POST(request: NextRequest) {
	...
	if (Number(chainId) !== CHAIN_ID) {
		return NextResponse.json({ error: "Only Base Mainnet (chainId 8453) is supported" }, { status: 400 });
	}
	...
	try {
		...
		const address = (contractAddress as string)?.toLowerCase() === RETROPUNKS_CONTRACT.toLowerCase() ? RETROPUNKS_CONTRACT : (contractAddress as `0x${string}`);
