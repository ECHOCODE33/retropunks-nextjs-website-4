      ...
      try {
        ...
        const tokenData = await Promise.all(
          owned.map(async (nft: { id: { tokenId: string } }) => {
            const tokenId = nft.id.tokenId.replace(/^0x/, "").padStart(64, "0");
            ...
            try {
              ...
              // globalTokenMetadata: [tokenIdSeed, backgroundIndex, name (bytes32), bio]
              currentBg = meta ? Number(meta[1]) : 0;
              name = meta?.[2] != null ? bytes32ToString(String(meta[2])) : "";
              bio = meta?.[3] ?? "";
            } catch {
        ...
        setErrorMessage(
          err instanceof Error ? err.message : "Failed to load punks",
        );
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="w-full min-h-[60vh] flex flex-col items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-400 text-sm">
            You don't own any RetroPunks. Mint one to get started and customize
    ...
    <div className="w-full min-w-0 py-8 sm:py-12 lg:py-16 px-3 sm:px-4">
      <div className="max-w-6xl mx-auto min-w-0">
        <header className="text-center mb-8 sm:mb-12 lg:mb-16">
          <h1 className="section-title text-retro-orange mb-4 sm:mb-6">
            My RetroPunks
          </h1>
          <p className="text-gray-400 text-sm mt-2">
            {tokens.length} {tokens.length === 1 ? "punk" : "punks"} in your
            collection
          </p>
        </header>
                  ...
                  prev.map((t) =>
                    t.tokenId === id
                      ? {
                          ...
                          currentBg: updates.currentBg ?? t.currentBg,
                        }
                      : t,
                  ),
                );
