				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 lg:gap-10 justify-items-center">
					{tokens.map((token) => (
						<PunkCard
							...
							name={token.name}
							bio={token.bio}
							onMetadataUpdate={async (id, updates) => {
								setTokens((prev) =>
									prev.map((t) =>
										t.tokenId === id ? { ...t, name: updates.name ?? t.name, bio: updates.bio ?? t.bio, currentBg: updates.currentBg ?? t.currentBg } : t,
									),
								);
								// Refetch tokenURI so download uses current on-chain image (e.g. after background/name/bio change)
								try {
									const uriRes = await fetch("/api/read-contract", {
										method: "POST",
										body: JSON.stringify({
